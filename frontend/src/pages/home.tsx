...
const Home: React.FC = () => {
  ...
  const goInfo = (data: string) => {
    ...
    const { isLoading:RankIsLoadaing, data:RankData} = useQuery('getRank1CompanyList', getRank1CompanyList);
    const { isLoading:BookIsLoading, data:BookData} = useQuery('getBookRankCompanyList', getBookRankCompanyList);
            ...
            <div className='home'>
                ...
                <Typography sx={{fontSize:20, my:5}} textAlign='center'>실시간 급상승 검색 기업 top5 !</Typography>
                    ...
                    {Object.keys(RankData||{}).map((value:any, index:any) => (
                        <Grid item={true} xs={2} sm={2} md={2} key={index} onClick={() => { goInfo(RankData[value]['cname']) }}>
                                    ...
                                    <CardContent>
                                        ...
                                        <Box sx={{ m: 2 }}>
                                            ...
                                            <Stack direction="row" spacing={1}>
                                                <Box borderRadius={1} sx={{ width:75, height:30, border:"solid 1px black"}}>
                                                ...
                                                <Box borderRadius={1} sx={{ padding:'auto', width:75, height:30, border:"solid 1px black"}}>
                                                    <Typography gutterBottom variant="body1" sx={{ fontSize:15, marginLeft:1.5, marginTop:0.5 }}>
                                                        {
                    ...
                    {Object.keys(BookData||{}).map((value:any, index:any) => (
                        <Grid item={true} xs={2} sm={2} md={2} key={index} onClick={() => { goInfo(BookData[value]['cname']) }}>
                            <Card>
                                <CardActionArea>
                                    <CardContent>
                                    <CardMedia
